...
def _self_critique(
    ...
    # ── Check 1: any HIGH task skipped that could still fit? ─────────────────
    ...
    # ── Check 2: time conflicts among selected tasks ──────────────────────────
    # Mirrors the conflicts_with_selected logic in Schedule.generate().
    selected_intervals: List[tuple] = []  # (start_min, end_min, title)
    for task in selected_tasks:
        if task.start_time is None:
            continue
        t_start = task.start_time.hour * 60 + task.start_time.minute
        t_end = t_start + task.duration_minutes
        for sel_start, sel_end, sel_title in selected_intervals:
            if t_start < sel_end and sel_start < t_end:
                issues.append(
                    f"Time conflict: '{task.title}' overlaps '{sel_title}' — "
                    f"do not select both (pick the higher-priority one)."
                )
                break  # one report per conflicting task is enough
        selected_intervals.append((t_start, t_end, task.title))

    # ── Check 3: model concerns about HIGH-priority issues only ──────────────
    # Concerns about skipped LOW/MEDIUM tasks are expected and should not
    # trigger another revision round.
    clean = concerns.strip().lower()
    if (
        clean
        and clean not in ("none", "none.", "no concerns", "no concerns.", "n/a", "n/a.")
        and "high" in clean
    ):
...
def run_agentic_schedule(
    ...
    # Track the best result seen: last iteration where no HIGH task was skipped.
    # Later iterations may regress (e.g. introduce conflicts), so we preserve
    # the best rather than always using the final iteration.
    best_selected_nums: List[int] = []
    best_reasoning = ""
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        # Save this iteration as the best if it has no HIGH-task gaps and no
        # time conflicts.  Later iterations may regress on either dimension, so
        # we keep the first clean result rather than always using the last one.
        if "was skipped but" not in critique and "Time conflict:" not in critique:
            best_selected_nums = prev_selected_nums[:]
            best_reasoning = prev_reasoning
    ...
    # Use the best result (last iteration with full HIGH coverage) rather than
    # the final iteration, which may have regressed.
    final_nums = best_selected_nums if best_selected_nums else prev_selected_nums
    final_reasoning = best_reasoning if best_reasoning else prev_reasoning
